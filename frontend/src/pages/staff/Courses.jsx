...
export default function AdminCourses() {
  ...
  const fetchCourses = async () => {
    try {
      const res = await api.get(`/api/courses/filter?department=${filter.department}&class_name=${filter.class_name}&batch=${filter.batch}`);
      setCourses(res.data);
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <Layout>
      <div>
        ...
        {/* Filter Bar */}
        <div className="bg-white rounded-2xl shadow p-4 mb-6 flex gap-3 items-center">
          <select value={filter.department}
            onChange={(e) => setFilter({ ...filter, department: e.target.value })}
            className="border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
            {DEPARTMENTS.map(d => <option key={d} value={d}>{d}</option>)}
          </select>
          <input type="text" placeholder="Class (A/B)"
        ...
        {/* Course Cards - Google Classroom Style */}
        {loading ? (
      ...
      {showUpload && (
        <UploadModal onClose={() => setShowUpload(false)} onUploaded={fetchCourses} />
      )}
      {confirmDelete && (
        <ConfirmModal
